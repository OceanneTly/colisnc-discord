                        ...
                        if (detailColis.getLocalizationLongName() == null || detailColis.getLocalizationUrl() == null) {
                            event.getChannel()
                                    .sendMessage("```txt\n" + "\u2714\uFE0F" + "Colis num° " + detailColis.getItemId()
                                            ...
                                            + detailColis.getTypeEvenement() + " et à déjà été récupéré.\n" + "```");
                        } else {
                            if (detailColis.getCountryCode().equals("NC")) {
                                // le print
                                event.getChannel()
                                        .sendMessage("```txt\n" + "\u2714\uFE0F" + "Colis num° "
                                                + detailColis.getItemId() + " :\n" + detailColis.getRawDateHeure()
                                                + "\n" + detailColis.getTypeEvenement() + " et est récupérable à l'"
                                                + detailColis.getLocalizationLongName() + " en "
                                ...
                                event.getChannel()
                                        .sendMessage("```txt\n" + "\u2708\uFE0F " + "Colis num° "
                                                + detailColis.getItemId() + " :\n" + detailColis.getRawDateHeure()
                                                + "\n" + detailColis.getTypeEvenement() + " et se trouve en"
                                                + detailColis.getCountryName() + "\n\n\uD83D\uDDFA\uFE0F Localisation :"
                                                + "```" + detailColis.getLocalizationUrl());
                            }
                        }
